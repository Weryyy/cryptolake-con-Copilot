...
def fetch_market_overview():
    try:
        response = requests.get(f"{API_URL}/api/v1/analytics/market-overview", timeout=15)
...
def fetch_fear_greed():
    try:
        response = requests.get(f"{API_URL}/api/v1/analytics/fear-greed", timeout=15)
        return response.json()
    except Exception as e:
...
def fetch_price_history(coin_id: str):
    try:
        response = requests.get(f"{API_URL}/api/v1/prices/{coin_id}", timeout=15)
        return response.json()
    except Exception as e:
...
def fetch_prediction():
    try:
        response = requests.get(f"{API_URL}/api/v1/analytics/prediction", timeout=15)
        return response.json()
    except Exception:
        return None
...
def fetch_dual_prediction():
    ...
    try:
        response = requests.get(f"{API_URL}/api/v1/analytics/dual-prediction", timeout=15)
        return response.json()
    except Exception:
        return None
...
def fetch_model_comparison():
    ...
    try:
        response = requests.get(f"{API_URL}/api/v1/analytics/model-comparison", timeout=15)
        return response.json()
    except Exception:
        return None
...
def fetch_prediction_accuracy():
    try:
        response = requests.get(f"{API_URL}/api/v1/analytics/prediction-accuracy", timeout=15)
        return response.json()
    except Exception:
        return None
...
def fetch_fear_greed_history():
    ...
    try:
        response = requests.get(f"{API_URL}/api/v1/analytics/fear-greed-history", timeout=15)
        return response.json()
    except Exception:
        return []
...
def fetch_system_alerts():
    try:
        response = requests.get(f"{API_URL}/api/v1/analytics/system-alerts", timeout=15)
        return response.json()
    except Exception:
        return []
...
def fetch_dq_reports():
    try:
        response = requests.get(f"{API_URL}/api/v1/analytics/dq-reports", timeout=15)
        return response.json()
    except Exception:
        return []
        ...
        if ohlc_data and len(ohlc_data) > 0:
            ...
            if dual and dual.get("ensemble"):
                ...
                if curve and len(curve) > 0:
                    ...
                    for pt in curve:
                        t = last_time + pd.Timedelta(seconds=pt["horizon_seconds"])
...
if page == "Market Overview":
    ...
    if not overview_data:
        ...
        if pred and pred["timestamp"] > 0:
            st.info(f"Analyzing Target: **{pred.get('coin_id', 'Unknown').upper()}**")
            p_col1, p_col2, p_col3 = st.columns(3)
            with p_col1:
                st.metric("Current Price (Ref)", f"${pred['current_price']:,.2f}")
            with p_col2:
    ...
    if not overview_data:
        ...
        if selected_coin:
            ...
            if ohlc_data:
                ...
                if selected_coin == "bitcoin":
                    ...
                    if dual and dual.get("ensemble"):
                        ...
                        if curve:
                            ...
                            cp = [last_price]
                            for pt in curve:
                                ct.append(last_time + pd.Timedelta(seconds=pt["horizon_seconds"]))
                ...
                st.plotly_chart(fig_candlestick, width="stretch", key="detail_candle")
            else:
                st.info(f"No hay datos de velas en tiempo real para {selected_coin} todavía.")
            ...
            if history:
                ...
                fig_line.update_layout(template="plotly_dark", uirevision="history-line")
    ...
    if not overview_data or len(overview_data) < 2:
        ...
        with comp_col2:
            coin_b = st.selectbox("Coin B", coins, index=min(1, len(coins) - 1))
        ...
        if coin_a and coin_b:
            ...
            if hist_a and hist_b:
                ...
                if len(df_a) > 0 and len(df_b) > 0:
                    df_a["normalized"] = (df_a["price_usd"] / df_a["price_usd"].iloc[0]) * 100
                    df_b["normalized"] = (df_b["price_usd"] / df_b["price_usd"].iloc[0]) * 100
                ...
                met_a = next((c for c in overview_data if c["coin_id"] == coin_a), {})
                met_b = next((c for c in overview_data if c["coin_id"] == coin_b), {})
        ...
        if dual:
            for label, key, _color_tag in [
                ...
                if m and m.get("predicted_price", 0) > 0:
                    dp = (
                        ((m["predicted_price"] - m["current_price"]) / m["current_price"]) * 100
                        if m.get("current_price", 0) > 0
                        else 0
                    )
                    icon = "🟢" if dp > 0 else "🔴"
                    st.metric(f"{label}", f"${m['predicted_price']:,.2f}", delta=f"{dp:+.2f}%")
            ...
            st.metric("AI Target", f"${pred['predicted_price']:,.2f}", delta=f"{diff_pct:+.2f}%")
    ...
    if overview_data:
        ...
        st.dataframe(pd.DataFrame(signal_data), hide_index=True, width="stretch")
    ...
    st.info("Detected GPU: **NVIDIA Quadro K4200 (Kepler)** | CPU: **Intel Xeon E5-1620 v3**")
    st.caption(
