...
Validators for each layer of the Lakehouse:
- BronzeValidator: raw data checks (existence, schema, freshness, row counts)
- SilverValidator: cleaned data checks (duplicates, nulls, ranges, positive prices)
- GoldValidator: star schema checks (PKs, referential integrity, fact rows)

Adapted from reference architecture for our actual Iceberg schema.

Usage:
    spark = SparkSession.builder.getOrCreate()
    bronze = BronzeValidator(spark)
    results = bronze.check_all()
    print(bronze.get_summary())
"""
...
class BronzeValidator(BaseValidator):
    ...
    def _check_freshness(self, table: str, ts_col: str):
        """Verify data is not older than 48 hours."""
        try:
...
class SilverValidator(BaseValidator):
        ...
        if not self._exists(table):
            self._add(
                CheckResult(
                    ...
                    table_name=table,
                    status=CheckStatus.WARNING,
                    message="NOT FOUND (may use bronze.fear_greed_index instead)",
                )
            )
            return
    ...
    def _check_realtime_vwap(self):
        table = "cryptolake.silver.realtime_vwap"
        if not self._exists(table):
            self._add(
                CheckResult(
                    check_name="table_exists",
        ...
        self._add(
            CheckResult(
                check_name="positive_ohlc",
                layer="silver",
                table_name=table,
                status=CheckStatus.PASSED if neg == 0 else CheckStatus.FAILED,
                metric_value=float(neg),
                threshold=0.0,
                message=f"Non-positive OHLC: {neg}",
            )
        )
...
class GoldValidator(BaseValidator):
    ...
    def _check_dim_dates(self):
        ...
        # Continuity: check for date gaps
        row = self.spark.sql(f"""
