"""
Spark Batch Job: Silver -> Gold (Star Schema)
...
def build_dim_coins(spark: SparkSession):
    ...
    spark.sql("""
        ...
        SELECT
            ...
            ROUND(
                ((MAX(price_usd) - MIN(price_usd)) / NULLIF(MIN(price_usd), 0)) * 100,
                2
            )                                   AS price_range_pct,
...
def build_fact_market_daily(spark: SparkSession):
    """Build fact_market_daily with technical indicators and sentiment.
    ...
    try:
        ...
        # No silver.fear_greed table — skip sentiment columns
        print("  ⚠️ silver.fear_greed not found, building without sentiment")
        fg_join = ""
        fg_select = """
            NULL AS fear_greed_value,
            NULL AS market_sentiment,
        """
        fg_combined = """
            'HOLD' AS combined_signal,
        """
        ...
        WITH price_metrics AS (
            SELECT
                ...
                ROUND(
                    ...
                    / NULLIF(LAG(p.price_usd, 1) OVER w_coin, 0) * 100,
                    4
